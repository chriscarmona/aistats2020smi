...
% \VignetteIndexEntry{ Supplementary material to Carmona and Nicholls (2020) Semi-Modular Inference }
% \VignetteDepends{ abind, cowplot, doParallel, doRNG, foreach, GPfit, ggplot2, ggcorrplot, ggmcmc, ggridges, dplyr, magrittr, mvtnorm, stringr, tidyr }
% \VignetteKeyword{AISTATS}
% \VignetteKeyword{SMI}
% \VignetteKeyword{Supplement}
...
req.pck <- c( "magrittr","ggplot2","dplyr","tidyr",
...
In this document we present additional material which complements the main article. The section numbers in this document are aligned with the sections in the main article, for the ease of referencing.
...
All figures and numerical results presented in the main text and this supplement can be replicated using our R package, \texttt{aistats2020smi}, available in Github.
...
Similarly, from Eq.~\ref{eq:smi_post} we can derive the loss function underlying the update of beliefs in SMI the posterior,
...
\begin{figure}[!ht]
  \center
  \includegraphics[width=0.5\textwidth]{SMI_biased_MSE_average}
  \caption{Mean Squared Error of the two main parameters under SMI posterior.}
  \label{fig:SMI_biased_MSE_average}
\end{figure}

Here we also show a comparison between $\theta$ and $\tilde\theta$. Figure~\ref{fig:SMI_biased_MSE_average_theta} shows that $\tilde\theta$ is dominated by $\theta$ in MSE. The comparison emphasises the convenience of SMI over power likelihood discussed in section 4.4 of the main text.

\begin{figure}[!ht]
  \center
  \includegraphics[width=0.5\textwidth]{SMI_biased_MSE_average_theta}
...
\begin{figure}[!ht]
  \center
  \includegraphics[width=0.5\textwidth]{SMI_biased_elpd}
...
<<agric_smi_init,eval=TRUE,echo=FALSE>>=
options(scipen=999, stringsAsFactors=FALSE)
set.seed(0)
out_dir <- '../inst'
...
# Levels of influence in Semi-Modular Inference
...
<<agric_smi_mcmc,eval=TRUE,echo=FALSE>>=
compute_mcmc = FALSE
n_iter_mcmc = 500e3
...
<<agric_smi_mcmc_summary,eval=TRUE,echo=FALSE>>=
...
if( compute_summary ) {
  ...
  # Joy Division style #
  param_joy = c("gamma_po_1","alpha_po_1","alpha_po_2","sigma_eta_PO")
  agric_smi_post_all = foreach( eta_PO_i = seq_along(power_eta_all), .errorhandling="pass", .combine="rbind" ) %dopar% {
    # dataset_i=1
    ...
    if( file.exists(file_i) ) {
      ...
      gamma_post_i = data.frame( arc_dataset=arc_dataset[dataset_i],
                                 ...
                                 mcmc_res[["theta_mcmc"]][,param_joy,drop=F]%>%as.data.frame() )
    }
    gamma_post_i
  }
  agric_smi_post_all$arc_dataset = factor(agric_smi_post_all$arc_dataset,levels=c("NMeso"))
  save( agric_smi_post_all, file="../data/agric_smi_post_all.rda" )
}
@
...
if(plot_mcmc) {
  ...
  ggsave( plot=p_gamma_interv,
          filename="gamma_po_1_smi_interval_alldatasets.pdf",
  ...
  param_joy = "gamma_po_1"
  foreach::foreach( param_i = param_joy ) %dopar% {
    # param_i = param_joy[1]
    # cat(param_i,"\n")
  ...
  p_gamma_neg <- BF_data %>%
    ...
    coord_cartesian( xlim=c(0,1) ) +
    labs( x="eta", y="Bayes Factor for gamma < 0" )+
    theme(legend.position="none") # Remove legend
  # print(p_gamma_neg)
...
<<agric_smi_model_select,eval=TRUE,echo=FALSE>>=
...
if( plot_model_eval ){
  ...
  # print(p_elpd)
  ggsave( plot=p_elpd,
          filename="agric_smi_model_eval_elpd_waic.pdf",
          device="pdf", width=12,height=8, units="cm")
...
\begin{figure}[!ht]
  \center
  \includegraphics[width=0.7\textwidth]{agric_smi_post_gamma_po_1}
  \caption{SMI posterior distribution for the parameter of interest in the agricultural model.}
...
The elpd changes dramatically over the range $0\le \eta\le 1$ and $\eta^*$ is clearly distinguished from 0 or 1. This impacts downstream inference: the Bayes factor of interest changes from $BF_{smi,0}(\gamma \leq 0)=$\Sexpr{round(BF_data[BF_data$eta_PO==0.00,"BF"],2)} in the cut model, to $BF_{smi,0.82}(\gamma \leq 0)=$\Sexpr{round(BF_data[BF_data$eta_PO==elpd_gp_best[elpd_gp_best$arc_dataset=="NMeso","eta_PO"],"BF"],2)} in the optimal SMI posterior with $\eta=$\Sexpr{round(elpd_gp_best[elpd_gp_best$arc_dataset=="NMeso","eta_PO"],4)}, a substantial shift in the strength of the evidence for the \emph{extensification} hypothesis. In Figure~\ref{fig:agric_smi_gamma_leq_0_BF} we show the comparison of Bayes Factors  accross values of $\eta \in [0,1]$ for the hypothesis $\gamma \leq 0$.
